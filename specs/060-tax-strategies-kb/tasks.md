...
- [X] T028 [US1] Implement `research_strategy`, `draft_strategy`, `enrich_strategy` Celery tasks in the same file. Phase 1 shipping behaviour:
  - **research** [X]: loads a pre-populated list of ATO source URLs from a per-strategy fixture map at `backend/app/modules/tax_strategies/data/ato_source_fixtures.py` (Phase 1 supplies entries for CLR-012 + CLR-241; Phase 2 replaces this with real ATO scraping). Writes `ato_sources` and transitions status stub→researching. Unit tests in `test_ato_source_fixtures.py` cover fixture shape + encapsulation.
  - **draft** [X]: real Anthropic SDK call (Claude Sonnet) using the prompt at architecture §10.3, extracted into `app/modules/tax_strategies/llm.py` with a parseable two-section output (`## Implementation` + `## Explanation`). Reads fixture-loaded `ato_sources`, emits `implementation_text` + `explanation_text`, transitions researching→drafted (or enriched→drafted on reject). Unit tests in `test_llm_prompts.py` cover prompt assembly + response parsing (tolerant of heading variations + code fences).
  - **enrich** [X]: second LLM pass extracting structured eligibility metadata (`entity_types`, `income_band_*`, `industry_triggers`, `financial_impact_type`, `keywords`, age bands). Defaults to NULL/empty on ambiguous fields per architecture §16 mitigations. Unit tests cover JSON parsing, enum filtering, dedupe/lowercase, and safe-default behaviour on malformed output.
...
- [X] T054 [US3] Status-count header implemented — reads `usePipelineStats`, renders non-zero statuses ordered by lifecycle sequence next to the Seed button.
...
- [X] T062 [US4] "Seed from CSV" button in `StrategiesTab` top bar — `AlertDialog` confirmation (explains idempotency + audit), toast on success with created/skipped counts, toast on failure with the backend error message.
