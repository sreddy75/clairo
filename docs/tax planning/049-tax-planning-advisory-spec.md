...
- **FR-001**: System MUST pull Profit & Loss data from Xero for the selected client and financial year, categorising income and expenses by tax-relevant types.
- **FR-002**: System MUST calculate estimated tax position for Australian companies (25% / 30% rate) and individuals (2025–26 marginal rates) including Medicare Levy.
...
- **FR-007**: System MUST support manual entry of financials for clients without Xero connections.
- **FR-008**: System MUST persist tax plans per client per financial year with full conversation history.
...
- **TaxPlan**: A tax planning session for a specific client and financial year. Contains base financials, entity type, and status.
...
- **TaxRateConfig**: Australian tax rates, thresholds, and offsets stored as configuration data (not hardcoded). Includes individual rates, company rates, Medicare Levy, LITO, HELP repayment thresholds.
