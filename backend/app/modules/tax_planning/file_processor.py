"""File processing for tax planning chat attachments.

Handles storage (MinIO) and content extraction for files attached to
chat messages. Extracted content is sent to Claude alongside the user's
text message; the original file is stored in MinIO for persistence.
...
async def process_chat_attachment(
    ...
    Args:
        ...
        plan_id: Tax plan ID for storage path.
    ...
    object_key = f"tax-planning/{tenant_id}/{plan_id}/chat/{message_id}{ext}"
