...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.String(length=20),
            nullable=True,
            comment="BAS status for non-Xero clients",
        ),
        ...
        sa.ForeignKeyConstraint(
            ["xero_connection_id"], ["xero_connections.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("xero_connection_id"),
    )
    op.create_index(
        "ix_practice_clients_assigned_user_id", "practice_clients", ["assigned_user_id"]
    )
    ...
    op.create_index(
        "ix_practice_clients_tenant_software",
        "practice_clients",
        ["tenant_id", "accounting_software"],
    )
    ...
    op.create_index(
        "ix_note_history_client_date",
        "client_note_history",
        ["client_id", sa.text("edited_at DESC")],
    )
    ...
    op.create_index(
        "ix_client_quarter_exclusions_tenant_id", "client_quarter_exclusions", ["tenant_id"]
    )
    op.create_index(
        "ix_exclusions_tenant_quarter",
        "client_quarter_exclusions",
        ["tenant_id", "quarter", "fy_year"],
    )
    op.create_index(
        "uix_client_quarter_exclusion_active",
        "client_quarter_exclusions",
        ["client_id", "quarter", "fy_year"],
        unique=True,
    ...
    op.add_column(
        ...
        sa.Column(
            ...
            nullable=True,
            comment="Cached display name from Clerk",
        ),
    )
