...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('tax_plan_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('tax_plans.id', ondelete='CASCADE'), nullable=False),
    ...
    op.create_index('ix_tax_plan_analyses_tenant_status', 'tax_plan_analyses', ['tenant_id', 'status'])
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False, server_default=sa.text('gen_random_uuid()')),
        sa.Column('tenant_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('tenants.id', ondelete='CASCADE'), nullable=False),
        sa.Column('analysis_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('tax_plan_analyses.id', ondelete='CASCADE'), nullable=False),
    ...
    op.create_index('ix_implementation_items_tenant_status', 'implementation_items', ['tenant_id', 'status'])

    # Add current_analysis_id FK to tax_plans
    op.add_column('tax_plans', sa.Column('current_analysis_id', postgresql.UUID(as_uuid=True), nullable=True))
