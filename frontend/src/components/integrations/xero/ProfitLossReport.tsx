'use client';

/**
 * ProfitLossReport Component
 *
 * Displays a Profit & Loss report from Xero with summary metrics
 * and detailed row breakdown.
 *
 * Spec 023: Xero Reports API Integration
 */

import {
  AlertCircle,
  ChevronDown,
  ChevronRight,
  Clock,
  DollarSign,
  Percent,
  RefreshCw,
  TrendingDown,
  TrendingUp,
} from 'lucide-react';
import { useState } from 'react';

import type { ProfitAndLossSummary, ReportResponse, ReportRow } from '@/lib/xero-reports';
import { formatCurrency, formatPercentage } from '@/lib/xero-reports';

interface ProfitLossReportProps {
  /** Report data from the API */
  report: ReportResponse | null;
  /** Whether the report is loading */
  isLoading?: boolean;
  /** Error message if report failed to load */
  error?: string | null;
  /** Callback to refresh the report */
  onRefresh?: () => void;
  /** Whether refresh is in progress */
  isRefreshing?: boolean;
  /** Additional CSS classes */
  className?: string;
}

/**
 * Metric card for displaying a single P&L metric
 */
function MetricCard({
  label,
  value,
  format = 'currency',
  trend,
  icon: Icon,
}: {
  label: string;
  value: number | null | undefined;
  format?: 'currency' | 'percentage';
  trend?: 'up' | 'down' | 'neutral';
  icon?: React.ComponentType<{ className?: string }>;
}) {
  const formattedValue =
    format === 'currency'
      ? formatCurrency(value)
      : formatPercentage(value);

  const trendColor =
    trend === 'up'
      ? 'text-status-success'
      : trend === 'down'
        ? 'text-status-danger'
        : 'text-muted-foreground';

  return (
    <div className="flex flex-col space-y-1">
      <span className="text-xs text-muted-foreground uppercase tracking-wide">
        {label}
      </span>
      <div className="flex items-center gap-2">
        {Icon && <Icon className={`h-4 w-4 ${trendColor}`} />}
        <span className={`text-xl font-semibold ${trendColor}`}>
          {formattedValue}
        </span>
      </div>
    </div>
  );
}

/**
 * Section for displaying report rows
 */
function ReportSection({
  title,
  rows,
  defaultOpen = false,
}: {
  title: string;
  rows: ReportRow[];
  defaultOpen?: boolean;
}) {
  const [isOpen, setIsOpen] = useState(defaultOpen);

  if (!rows || rows.length === 0) return null;

  return (
    <div className="border rounded-lg overflow-hidden">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="flex items-center justify-between w-full py-3 px-4 bg-muted hover:bg-muted text-left transition-colors"
      >
        <span className="font-medium text-foreground">{title}</span>
        {isOpen ? (
          <ChevronDown className="h-4 w-4 text-muted-foreground" />
        ) : (
          <ChevronRight className="h-4 w-4 text-muted-foreground" />
        )}
      </button>
      {isOpen && (
        <div className="border-t">
          <table className="w-full text-sm">
            <tbody>
              {rows.map((row, idx) => (
                <ReportRowDisplay key={idx} row={row} />
              ))}
            </tbody>
          </table>
        </div>
      )}
    </div>
  );
}

/**
 * Display a single report row
 */
function ReportRowDisplay({ row, depth = 0 }: { row: ReportRow; depth?: number }) {
  const isHeader = row.row_type === 'Header';
  const isSummary = row.row_type === 'SummaryRow';
  const isSection = row.row_type === 'Section';
  const cells = row.cells || [];

  if (isSection && row.title) {
    return (
      <>
        <tr className="bg-muted">
          <td
            colSpan={cells.length || 2}
            className="px-4 py-2 font-medium text-foreground"
            style={{ paddingLeft: `${depth * 16 + 16}px` }}
          >
            {row.title}
          </td>
        </tr>
        {row.rows?.map((childRow, idx) => (
          <ReportRowDisplay key={idx} row={childRow} depth={depth + 1} />
        ))}
      </>
    );
  }

  if (cells.length === 0) return null;

  const rowClasses = `
    border-b border-border last:border-0
    ${isHeader ? 'bg-muted font-medium' : ''}
    ${isSummary ? 'bg-muted font-medium' : ''}
  `;

  return (
    <tr className={rowClasses}>
      {cells.map((cell, idx) => (
        <td
          key={idx}
          className={`px-4 py-2 ${idx === 0 ? 'text-left' : 'text-right'}`}
          style={idx === 0 ? { paddingLeft: `${depth * 16 + 16}px` } : undefined}
        >
          {cell?.value || ''}
        </td>
      ))}
    </tr>
  );
}

/**
 * Loading skeleton for the report
 */
function ReportSkeleton() {
  return (
    <div className="space-y-6 animate-pulse">
      {/* Summary cards skeleton */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="space-y-2">
            <div className="h-4 w-24 bg-muted rounded" />
            <div className="h-8 w-32 bg-muted rounded" />
          </div>
        ))}
      </div>

      {/* Sections skeleton */}
      <div className="space-y-4">
        {[1, 2, 3].map((i) => (
          <div key={i} className="space-y-2">
            <div className="h-12 w-full bg-muted rounded" />
            <div className="h-32 w-full bg-muted rounded" />
          </div>
        ))}
      </div>
    </div>
  );
}

export function ProfitLossReport({
  report,
  isLoading = false,
  error = null,
  onRefresh,
  isRefreshing = false,
  className = '',
}: ProfitLossReportProps) {
  // Extract summary with type assertion
  const summary = report?.summary as ProfitAndLossSummary | undefined;

  // Group rows by section
  const sections = report?.rows?.filter(
    (row) => row.row_type === 'Section'
  ) || [];

  // Determine profit trends
  const netProfitTrend =
    summary?.net_profit && summary.net_profit > 0 ? 'up' : 'down';
  const grossMarginTrend =
    summary?.gross_margin_pct && summary.gross_margin_pct > 30 ? 'up' : 'neutral';

  if (isLoading) {
    return (
      <div className={`bg-card rounded-lg shadow-sm border p-6 ${className}`}>
        <div className="mb-4">
          <h3 className="text-lg font-semibold text-foreground">Profit & Loss</h3>
          <p className="text-sm text-muted-foreground">Loading report...</p>
        </div>
        <ReportSkeleton />
      </div>
    );
  }

  if (error) {
    return (
      <div className={`bg-card rounded-lg shadow-sm border p-6 ${className}`}>
        <div className="mb-4">
          <h3 className="text-lg font-semibold text-foreground flex items-center gap-2">
            <AlertCircle className="h-5 w-5 text-status-danger" />
            Profit & Loss
          </h3>
          <p className="text-sm text-status-danger">{error}</p>
        </div>
        {onRefresh && (
          <button
            onClick={onRefresh}
            disabled={isRefreshing}
            className="inline-flex items-center px-3 py-2 border border-border rounded-md text-sm font-medium text-foreground bg-card hover:bg-muted disabled:opacity-50"
          >
            <RefreshCw
              className={`h-4 w-4 mr-2 ${isRefreshing ? 'animate-spin' : ''}`}
            />
            Retry
          </button>
        )}
      </div>
    );
  }

  if (!report) {
    return (
      <div className={`bg-card rounded-lg shadow-sm border p-6 ${className}`}>
        <div className="mb-4">
          <h3 className="text-lg font-semibold text-foreground">Profit & Loss</h3>
          <p className="text-sm text-muted-foreground">No report data available</p>
        </div>
        {onRefresh && (
          <button
            onClick={onRefresh}
            className="inline-flex items-center px-3 py-2 border border-border rounded-md text-sm font-medium text-foreground bg-card hover:bg-muted"
          >
            Load Report
          </button>
        )}
      </div>
    );
  }

  return (
    <div className={`bg-card rounded-lg shadow-sm border ${className}`}>
      {/* Header */}
      <div className="flex items-start justify-between p-6 border-b">
        <div>
          <h3 className="text-lg font-semibold text-foreground flex items-center gap-2">
            {report.report_name}
            {report.is_stale && (
              <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-amber-100 text-amber-700">
                <Clock className="h-3 w-3 mr-1" />
                Stale
              </span>
            )}
          </h3>
          <p className="text-sm text-muted-foreground">
            {report.report_titles?.join(' • ') || 'Financial report'}
          </p>
        </div>
        {onRefresh && (
          <button
            onClick={onRefresh}
            disabled={isRefreshing}
            className="p-2 text-muted-foreground hover:text-foreground disabled:opacity-50"
          >
            <RefreshCw
              className={`h-4 w-4 ${isRefreshing ? 'animate-spin' : ''}`}
            />
          </button>
        )}
      </div>

      <div className="p-6 space-y-6">
        {/* Summary Metrics */}
        {summary && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 p-4 bg-muted rounded-lg">
            <MetricCard
              label="Total Income"
              value={summary.total_income}
              icon={DollarSign}
            />
            <MetricCard
              label="Gross Profit"
              value={summary.gross_profit}
              icon={TrendingUp}
              trend={summary.gross_profit > 0 ? 'up' : 'down'}
            />
            <MetricCard
              label="Net Profit"
              value={summary.net_profit}
              icon={netProfitTrend === 'up' ? TrendingUp : TrendingDown}
              trend={netProfitTrend}
            />
            <MetricCard
              label="Net Margin"
              value={summary.net_margin_pct}
              format="percentage"
              icon={Percent}
              trend={grossMarginTrend}
            />
          </div>
        )}

        {/* Additional Metrics Row */}
        {summary && (
          <div className="grid grid-cols-3 md:grid-cols-6 gap-3 text-sm">
            <div className="text-center p-3 border rounded-lg bg-card">
              <div className="text-muted-foreground text-xs">Revenue</div>
              <div className="font-medium text-foreground">{formatCurrency(summary.revenue)}</div>
            </div>
            <div className="text-center p-3 border rounded-lg bg-card">
              <div className="text-muted-foreground text-xs">Other Income</div>
              <div className="font-medium text-foreground">{formatCurrency(summary.other_income)}</div>
            </div>
            <div className="text-center p-3 border rounded-lg bg-card">
              <div className="text-muted-foreground text-xs">Cost of Sales</div>
              <div className="font-medium text-foreground">{formatCurrency(summary.cost_of_sales)}</div>
            </div>
            <div className="text-center p-3 border rounded-lg bg-card">
              <div className="text-muted-foreground text-xs">Operating Exp.</div>
              <div className="font-medium text-foreground">{formatCurrency(summary.operating_expenses)}</div>
            </div>
            <div className="text-center p-3 border rounded-lg bg-card">
              <div className="text-muted-foreground text-xs">Gross Margin</div>
              <div className="font-medium text-foreground">{formatPercentage(summary.gross_margin_pct)}</div>
            </div>
            <div className="text-center p-3 border rounded-lg bg-card">
              <div className="text-muted-foreground text-xs">Expense Ratio</div>
              <div className="font-medium text-foreground">{formatPercentage(summary.expense_ratio_pct)}</div>
            </div>
          </div>
        )}

        {/* Detailed Sections */}
        <div className="space-y-3">
          {sections.map((section, idx) => (
            <ReportSection
              key={idx}
              title={section.title || `Section ${idx + 1}`}
              rows={section.rows}
              defaultOpen={idx === 0}
            />
          ))}
        </div>

        {/* Report Metadata */}
        <div className="flex items-center justify-between text-xs text-muted-foreground pt-4 border-t">
          <span>
            Fetched: {new Date(report.fetched_at).toLocaleString('en-AU')}
          </span>
          <span>
            Period: {report.period_key}
          </span>
        </div>
      </div>
    </div>
  );
}

export default ProfitLossReport;
